...
func TestSvg(t *testing.T) {
	...
	var cases = []struct {
		...
		{
			"valid",
			httptest.NewRequest(http.MethodGet, "/times", nil),
			"<svg",
			http.StatusOK,
			http.Header{
	...
	for _, tc := range cases {
		t.Run(tc.intention, func(t *testing.T) {
			writer := httptest.NewRecorder()
			testApp.svg().ServeHTTP(writer, tc.request)

			if got := writer.Code; got != tc.wantStatus {
				t.Errorf("svg = %d, want %d", got, tc.wantStatus)
			}

			if got, _ := request.ReadBodyResponse(writer.Result()); !strings.Contains(string(got), tc.want) {
				t.Errorf("svg = `%s`, want `%s`", string(got), tc.want)
			}
